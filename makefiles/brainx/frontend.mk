...
# 使用 .env 中定义的端口
BRAINX_FRONTEND_PORT := $(BRAINX_FRONTEND_BACKEND_PORT)
...
ABS_BRAINX_FRONTEND_BACKEND_DIR:= $(call get_absolute_path,$(BRAINX_FRONTEND_BACKEND_DIR))
...
build.brainx:
	...
	${ABS_BRAINX_FRONTEND_BACKEND_DIR}

rerun.brainx.frontend: stop.brainx.frontend run.brainx.frontend

# 运行日志容器
run.brainx.backend.frontend:
	docker run -d \
		--name $(BRAINX_FRONTEND_IMAGE_NAME) \
		-p $(BRAINX_FRONTEND_PORT):8000 \
		-v $(ABS_BRAINX_FRONTEND_DATA_PATH):/app \
